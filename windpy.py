...
class Address(object):
    def __init__(self, address='', publicKey='', privateKey='', seed='', alias='', nonce=0):
        ...
        if seed:
            ...
            if not validateAddress(address):
                ...
                self.seed = seed
                self.nonce = nonce
        elif alias and not OFFLINE:
            self.address = wrapper('/alias/by-alias/%s' % alias).get("address", "")
            self.publicKey = ''
            self.privateKey = ''
            self.seed = ''
            self.nonce = 0
    ...
    def __str__(self):
        if self.address:
            ab = []
            try:
                assets_balances = wrapper('/assets/balance/%s' % self.address)['balances']
    ...
    def balance(self, assetId='', confirmations=0):
        try:
            if assetId:
                return wrapper('/assets/balance/%s/%s' % (self.address, assetId))['balance']
            else:
                return wrapper('/addresses/balance/%s%s' % (self.address, '' if confirmations==0 else '/%d' % confirmations))['balance']
        except:
            return 0

    def assets(self):
        req = wrapper('/assets/balance/%s' % self.address)['balances']
        return [r['assetId'] for r in req]

    def aliases(self):
        a = wrapper('/alias/by-address/%s' % self.address)
        if type(a)==list:
    ...
    def issueAsset(self, name, description, quantity, decimals=0, reissuable=False, txFee=DEFAULT_ASSET_FEE):
        if not self.privateKey:
            ...
            req = wrapper('/assets/broadcast/issue', data)
            if OFFLINE:
                return req
            else:
                return req['assetId']
    ...
    def reissueAsset(self, Asset, quantity, reissuable=False, txFee=DEFAULT_TX_FEE):
        timestamp = int(time.time() * 1000)
        sData = b'\5' + \
                base58.b58decode(self.publicKey) + \
        ...
        req = wrapper('/assets/broadcast/reissue', data)
        if OFFLINE:
            return req
        else:
            return req.get('id', 'ERROR')

    def burnAsset(self, Asset, quantity, txFee=DEFAULT_TX_FEE):
        timestamp = int(time.time() * 1000)
        ...
        req = wrapper('/assets/broadcast/burn', data)
        if OFFLINE:
            return req
        else:
            return req.get('id', 'ERROR')

    def sendWind(self, recipient, amount, attachment='', txFee=DEFAULT_TX_FEE, timestamp=0):
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            throw_error(msg)
            ...
            logging.error(msg)
            throw_error(msg)
        elif not OFFLINE and self.balance() < amount + txFee:
            ...
            return wrapper('/transactions/broadcast', data)

    def massTransferWaves(self, transfers, attachment='', timestamp=0,baseFee=DEFAULT_BASE_FEE):
        ...
        if not self.privateKey:
            ...
            logging.error(msg)
            throw_error(msg)
        elif not OFFLINE and self.balance() < totalAmount + txFee:
            msg = 'Insufficient Waves balance'
            logging.error(msg)
            throw_error(msg)
        else:
    ...
    def sendAsset(self, recipient, asset, amount, attachment='', feeAsset='', txFee=DEFAULT_TX_FEE, timestamp=0):
        if not self.privateKey:
            msg = 'Asset not issued'
            logging.error(msg)
            throw_error(msg)
        elif not OFFLINE and asset and not asset.status():
            msg = 'Asset not issued'
            logging.error(msg)
            throw_error(msg)
        elif amount <= 0:
            ...
            logging.error(msg)
            throw_error(msg)
        elif not OFFLINE and asset and self.balance(asset.assetId) < amount:
            msg = 'Insufficient asset balance'
            logging.error(msg)
            throw_error(msg)
        elif not OFFLINE and not asset and self.balance() < amount:
            msg = 'Insufficient Waves balance'
            logging.error(msg)
            throw_error(msg)
        elif not OFFLINE and not feeAsset and self.balance() < txFee:
            msg = 'Insufficient Waves balance'
            logging.error(msg)
            throw_error(msg)
        elif not OFFLINE and feeAsset and self.balance(feeAsset.assetId) < txFee:
            msg = 'Insufficient asset balance'
            logging.error(msg)
            throw_error(msg)
        else:
            if feeAsset:
                feeInfos = wrapper('/assets/details/' + feeAsset.assetId)
            ...
            return wrapper('/assets/broadcast/transfer', data)

    def massTransferAssets(self, transfers, asset, attachment='', timestamp=0,baseFee=DEFAULT_BASE_FEE,smartFee=DEFAULT_SMART_FEE):
        txFee = baseFee + (math.ceil((len(transfers) + 1) / 2 - 0.5)) * baseFee
        ...
        elif not OFFLINE and self.balance() < txFee:
            logging.error('Insufficient Waves balance')
        elif not OFFLINE and self.balance(assetId=asset.assetId) < totalAmount:
    ...
    def dataTransaction(self, data, timestamp=0, baseFee=DEFAULT_BASE_FEE, minimalFee=500000):
        if not self.privateKey:
            logging.error('Private key required')
        else:
            if timestamp == 0:
            ...
            return wrapper('/transactions/broadcast', dataObjectJSON)

    def _postOrder(self, amountAsset, priceAsset, orderType, amount, price, maxLifetime=30*86400, matcherFee=DEFAULT_MATCHER_FEE, timestamp=0):
        if timestamp == 0:
            timestamp = int(time.time() * 1000)
        ...
        sData = base58.b58decode(self.publicKey) + \
                base58.b58decode(MATCHER_PUBLICKEY) + \
                asset1 + \
                asset2 + \
                orderType + \
                struct.pack(">Q", price) + \
        ...
        data = json.dumps({
            "senderPublicKey": self.publicKey,
            "matcherPublicKey": MATCHER_PUBLICKEY,
            "assetPair": {
        ...
        req = wrapper('/matcher/orderbook', data, host=MATCHER)
        id = -1
        if 'status' in req:
    ...
    def cancelOrder(self, assetPair, order):
        ...
        req = wrapper('/matcher/orderbook/%s/%s/cancel' % (DEFAULT_CURRENCY if assetPair.asset1.assetId=='' else assetPair.asset1.assetId, DEFAULT_CURRENCY if assetPair.asset2.assetId=='' else assetPair.asset2.assetId), data, host=MATCHER)
        if OFFLINE:
            return req
        else:
            id = -1
    ...
    def buy(self, assetPair, amount, price, maxLifetime=30 * 86400, matcherFee=DEFAULT_MATCHER_FEE, timestamp=0):
        assetPair.refresh()
        ...
        if OFFLINE:
            ...
            return Order(id, assetPair, self)

    def sell(self, assetPair, amount, price, maxLifetime=30 * 86400, matcherFee=DEFAULT_MATCHER_FEE, timestamp=0):
        assetPair.refresh()
        normPrice = int(pow(10, 8 + assetPair.asset2.decimals - assetPair.asset1.decimals) * price)
    ...
    def tradableBalance(self, assetPair):
        try:
            req = wrapper('/matcher/orderbook/%s/%s/tradableBalance/%s' % (DEFAULT_CURRENCY if assetPair.asset1.assetId == '' else assetPair.asset1.assetId, DEFAULT_CURRENCY if assetPair.asset2.assetId == '' else assetPair.asset2.assetId, self.address), host=MATCHER)
            if OFFLINE:
                    return req
            amountBalance = req[DEFAULT_CURRENCY if assetPair.asset1.assetId == '' else assetPair.asset1.assetId]
            priceBalance = req[DEFAULT_CURRENCY if assetPair.asset2.assetId == '' else assetPair.asset2.assetId]
        except:
            amountBalance = 0
            priceBalance = 0
        if not OFFLINE:
    ...
    def lease(self, recipient, amount, txFee=DEFAULT_LEASE_FEE, timestamp=0):
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            throw_error(msg)
            ...
            req = wrapper('/leasing/broadcast/lease', data)
            return req

    def leaseCancel(self, leaseId, txFee=DEFAULT_LEASE_FEE, timestamp=0):
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            throw_error(msg)
            ...
            req = wrapper('/leasing/broadcast/cancel', data)
            if OFFLINE:
                return req
            elif 'leaseId' in req:
                return req['leaseId']
    ...
    def getOrderHistory(self, assetPair, timestamp=0):
        ...
        req = wrapper('/matcher/orderbook/%s/%s/publicKey/%s' % (DEFAULT_CURRENCY if assetPair.asset1.assetId=='' else assetPair.asset1.assetId, DEFAULT_CURRENCY if assetPair.asset2.assetId=='' else assetPair.asset2.assetId, self.publicKey), headers=data, host=MATCHER)
        return req
    ...
    def cancelOpenOrders(self, assetPair):
        ...
        for order in orders:
            ...
            if status=='Accepted' or status=='PartiallyFilled':
                ...
                wrapper('/matcher/orderbook/%s/%s/cancel' % (DEFAULT_CURRENCY if assetPair.asset1.assetId == '' else assetPair.asset1.assetId, DEFAULT_CURRENCY if assetPair.asset2.assetId == '' else assetPair.asset2.assetId), data, host=MATCHER)
    ...
    def deleteOrderHistory(self, assetPair):
        ...
        for order in orders:
            ...
            wrapper('/matcher/orderbook/%s/%s/delete' % (DEFAULT_CURRENCY if assetPair.asset1.assetId == '' else assetPair.asset1.assetId, DEFAULT_CURRENCY if assetPair.asset2.assetId == '' else assetPair.asset2.assetId), data, host=MATCHER)

    def createAlias(self, alias, txFee=DEFAULT_ALIAS_FEE, timestamp=0):
        aliasWithNetwork = b'\x02' + crypto.str2bytes(str(CHAIN_ID)) + struct.pack(">H", len(alias)) + crypto.str2bytes(alias)
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            throw_error(msg)
            ...
            return wrapper('/alias/broadcast/create', data)

    def sponsorAsset(self, Asset, minimalFeeInAssets, txFee=DEFAULT_SPONSOR_FEE, timestamp=0):
        if not self.privateKey:
            logging.error('Private key required')
        else:
            if timestamp == 0:
    ...
    def setScript(self, scriptSource, txFee=DEFAULT_SCRIPT_FEE, timestamp=0):
        print(wrapper('/utils/script/compile', scriptSource));
        script = wrapper('/utils/script/compile', scriptSource)['script'][7:]
        if not self.privateKey:
            logging.error('Private key required')
        else:
    ...
    def setAssetScript(self, asset, scriptSource, txFee=DEFAULT_ASSET_SCRIPT_FEE, timestamp=0):
        script = wrapper('/utils/script/compile', scriptSource)['script'][7:]
        if not self.privateKey:
            logging.error('Private key required')
        else:
    ...
    def issueSmartAsset(self, name, description, quantity, scriptSource, decimals=0, reissuable=False, txFee=DEFAULT_ASSET_FEE):
        script = wrapper('/utils/script/compile', scriptSource)['script'][7:]
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            ...
            req = wrapper('/transactions/broadcast', data)
            if OFFLINE:
                return req
            else:
                return req

    def invokeScript(self, dappAddress, functionName, params, payments, feeAsset = None, txFee=DEFAULT_INVOKE_SCRIPT_FEE):
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            throw_error(msg)
